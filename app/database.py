...
def init_db():
    ...
    with get_db() as conn:
        ...
        # ── Migración segura: añadir columnas a tabla existente ─────────────
...
def authenticate_user(username_or_email: str, password: str) -> dict | None:
    """Verifica credenciales. Migra SHA-256 a bcrypt automáticamente."""
    with get_db() as conn:
        user = conn.execute(
...
def add_chat_file(chat_id: int, user_id: int, filename: str, text_content: str,
...
def get_latest_agent_session() -> dict | None:
    with get_db() as conn:
        row = conn.execute(
            "SELECT * FROM agent_sessions ORDER BY created_at DESC LIMIT 1").fetchone()
    return dict(row) if row else None
...
def save_agent_message(session_id: int, agent_id: str, content: str, msg_type: str = "chat") -> int:
    with get_db() as conn:
        cur = conn.execute(
...
def assign_pioneer_plan(user_id: int) -> dict:
    with get_db() as conn:
        ...
        return {"plan": plan, "pioneer_number": pioneer_number, "is_pioneer": pioneer_number is not None}
...
def use_redeem_code(user_id: int, code: str) -> dict:
    ...
    with get_db() as conn:
        row = conn.execute(
            "SELECT id, plan, used FROM redeem_codes WHERE code = ?", (code,)).fetchone()
        if not row:
        ...
        conn.execute("UPDATE redeem_codes SET used=1, used_by=?, used_at=? WHERE code=?",
                     (user_id, now, code))
        conn.execute("UPDATE users SET plan=?, plan_assigned_at=? WHERE id=?",
...
def get_user_by_api_key(raw_key: str) -> dict | None:
    ...
    with get_db() as conn:
        ...
        if not row or not row["is_active"]:
            return None
        conn.execute("UPDATE api_keys SET last_used_at = datetime('now') WHERE id = ?",
                     (row["key_id"],))
...
def get_user_selected_model(user_id: int) -> str:
    """Devuelve el ID del modelo Nexo seleccionado por el usuario. Default: 'nexo_coder'"""
    with get_db() as conn:
        row = conn.execute(
            "SELECT selected_model FROM users WHERE id = ?", (user_id,)).fetchone()
    if row and row["selected_model"]:
        return row["selected_model"]
    return "nexo_coder"


def set_user_selected_model(user_id: int, model_id: str) -> bool:
    """Guarda la selección de modelo Nexo del usuario (nexo_lite/nexo_coder/nexo_pro)."""
